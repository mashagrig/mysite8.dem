...
//        }
        ...
        //вопрос от незареганного пользователя
       if(Content::where('contents.status', 'like', "%{$email}%")->first() !== null){
           $question_from_contacts_all =
               Content::select(
                   ...
                   ->where('contents.status', 'like', "%{$email}%")
                   ->orderBy('contents.updated_at', 'desc')
                   ->groupBy('contents.id')
                   ->get();
       }  //вопрос от зареганного пользователя
       else if(Content::where('contents.status', 'like', "%{$email}%")->first() === null){
           $question_from_contacts_all =
               User::select(
